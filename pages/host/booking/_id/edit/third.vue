<template>
  <div>
    <HostSubHeader :title="'예약수정하기'" :link="'/host/booking/' + this.$route.params.id + '/edit/second'" />
    <div class="f_width user_padding">
      <div class="host_area layout_format">
        <div class="user_nik">
          <p><span>ON<span>TIME</span></span> 예약 프로그램 수정하기<br>예약수정 진행중 (3/4)</p>
        </div>
    ...
    async nextForm() {
      ...
      try {
        ...
        if (response.data.result) {
          ...
          this.$router.push('/host/booking/' + this.$route.params.id + '/edit/fourth');
        }
        this.loading = false;
      } catch (e) {
