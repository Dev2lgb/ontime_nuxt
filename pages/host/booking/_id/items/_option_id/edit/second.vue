<template>
  <div>
    <HostSubHeader :title="'예약상품 수정'" :link="'/host/booking/' + this.$route.params.id + '/items/' + this.$route.params.option_id + '/edit/'" />
    <div class="f_width user_padding">
      <div class="host_area layout_format">
        <div class="user_nik">
          <p><span>ON<span>TIME</span></span> 예약 상품 수정하기<br>상품수정 진행중 (2/4)</p>
        </div>
    ...
    async nextForm() {
      ...
      try {
        ...
        if (response.data.result) {
          ...
          this.$router.push('/host/booking/' + this.$route.params.id + '/items/' + this.$route.params.option_id + '/edit/third');
        }
        this.loading = false;
      } catch (e) {
